...
back_color = imread(r'e:\dataset\bg.jpg')  # 解析该图片

wc = WordCloud(background_color='white',  # 背景颜色
               max_words=1000,  # 最大词数
               mask=back_color,  # 以该参数值作图绘制词云，这个参数不为空时，width和height会被忽略
               max_font_size=100,  # 显示字体的最大值
               stopwords=STOPWORDS.add('广东'),  # 使用内置的屏蔽词，再添加'苟利国'
...
# 打开词源的文本文件
text = open(r'e:\dataset\didian2.txt').read()
...
def stop_words(texts):
    ...
    with open(r'e:\dataset\stopwords.txt') as f:
        str_text = f.read()
...
plt.axis('off')
# 保存图片
wc.to_file(r'e:/dataset/19th.png')